...
class SetValuedClassification:
    ...
    def StrongDominance(self) -> list[list[int]]:
        pred = []
        for i in range (self._nb_samples):
            binaries = [False] * self._initial_nb_classes
            focals, masses = Utils.ProbabilityToBelief(self._masses[i], self._distinct_relabeled_classes)
            ...
            for a in range(self._initial_nb_classes):
                ...
                if comp == (self._initial_nb_classes - 1):
                    binaries[a] = True
            pred.append(Utils.BinaryToClass(binaries))
        return pred
        ...
        for i in range (self._nb_samples):
            ...
            for j in range(len(focals)):
                ...
                for k in range(len(focals)):
                    B = Utils.BinaryToClass(Utils.IntegerToBinary(focals[k], self._initial_nb_classes)) 
            ...
            pred.append(Utils.BinaryToClass(Utils.IntegerToBinary(focals[np.argmax(gain)], self._initial_nb_classes)))
        return pred


    def intervalCriterion(self):
        pass